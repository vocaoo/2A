...
class CheckTask(Command[None]):
    task_id: UUID


class CheckTaskHandler(CommandHandler[CheckTask, None]):
    def __init__(
        self,
        task_repo: TaskRepo,
        uow: UnitOfWork,
    ...
    async def __call__(self, command: CheckTask) -> None:
        ...
        logger.info("Task checked", extra={"task": task})
