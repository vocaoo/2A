...
class DelayTask(Command[None]):
    task_id: UUID


class DelayTaskHandler(CommandHandler[DelayTask, None]):
    def __init__(
        self,
        task_repo: TaskRepo,
        uow: UnitOfWork,
    ...
    async def __call__(self, command: DelayTask) -> None:
        ...
        logger.info("Task delayed", extra={"task": task})
