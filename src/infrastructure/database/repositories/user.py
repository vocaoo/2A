...
from src.application.common.pagination.dto import (
    Pagination,
    PaginationResult,
    SortOrder,
)
...
from src.infrastructure.database.converters.user import (
...
class UserReaderImpl(SQLAlchemyRepo, UserReader):
    ...
    @exception_mapper
    async def get_users(
        self, filters: GetUserFilters, pagination: Pagination
    ) -> dto.Users:
        query = select(User)
        ...
        return dto.Users(
            data=users,
            pagination=PaginationResult.from_pagination(pagination, total=users_count),
        )
...
class UserRepoImpl(SQLAlchemyRepo, UserRepo):
    ...
    async def acquire_user_by_id(self, user_id: UserID) -> entities.User:
        user: User | None = await self._session.get(
            User, user_id.to_raw(), with_for_update=True
        )
        if user is None:
    ...
    async def get_existing_usernames(self) -> set[Username]:
        result: Iterable[str] = await self._session.scalars(
